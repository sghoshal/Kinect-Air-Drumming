...
static float crash_left_xmin = -100.00,crash_left_xmax = -40.00,crash_left_ymin = 40.00,crash_left_ymax = 70.00;

static float high_hat_xmin = 14.00 ,high_hat_xmax = 50.00,high_hat_ymin = 100.00 ,high_hat_ymax = 130.00;

static float ride_xmin = 148.00 ,ride_xmax = 200.00,ride_ymin = 45.00 ,ride_ymax = 100.00;

static float snare_xmin = -40.00 ,snare_xmax = 20.00,snare_ymin = 130.00, snare_ymax = 170.00;

static float lowtom_xmin = -180.00 ,lowtom_xmax = -20.00,lowtom_ymin = 120.00, lowtom_ymax = 140.00;

static float hitom_xmin = 0.00 ,hitom_xmax = 180.00,hitom_ymin = 120.00, hitom_ymax = 140.00;
...
{      
	...
	USHORT rel_right_depth = depth[2] - depth[10];
	USHORT rel_left_depth = depth[2] - depth[6];
	...
	//DBOUT("SHOULDER" << depth[2] << "\n");
	//DBOUT("LEFT" << depth[6] << "\n");
	//DBOUT("RIGHT" << depth[10] << "\n");


	//DBOUT("RIGHT" << rel_right_depth << "\n");
	//DBOUT("LEFT" << rel_left_depth << "\n");
	 //drums location
	float rel_left_x,rel_left_y,rel_right_x,rel_right_y;

	if((m_Points[6].x - left_x_old) > 0)
		left_x_pos = true;
	else
		left_x_pos = false;

	if((m_Points[6].y - left_y_old) > 0)
		left_y_pos = true;
	else
		left_y_pos = false;

	if((m_Points[10].x - right_x_old) > 0)
		right_x_pos = true;
	else
		right_x_pos = false;

	if((m_Points[10].y - right_y_old) > 0)
		right_y_pos = true;
	else
		right_y_pos = false;

	left_x_old = m_Points[6].x;
	left_y_old = m_Points[6].y;
	right_x_old = m_Points[10].x;
	right_y_old = m_Points[10].y;



	rel_left_x = m_Points[6].x - m_Points[2].x;
	rel_left_y = m_Points[6].y - m_Points[2].y;

	rel_right_x = m_Points[10].x - m_Points[2].x;
	rel_right_y = m_Points[10].y - m_Points[2].y;

	DBOUT("LEFT (" << rel_left_x << "," << rel_left_y << ")\n");
	DBOUT("RIGHT (" << rel_right_x << "," << rel_right_y << ")\n");
	if (rel_left_depth > 2500 )
	{
	...
	if ((rel_left_x < snare_xmax && rel_left_x > snare_xmin) && (rel_left_y < snare_ymax && rel_left_y > snare_ymin) && left_y_pos
		&& rel_left_depth <= 2000)
	{
		DBOUT("Snare played \n");
	...
	if ((rel_right_x < ride_xmax && rel_right_x > ride_xmin) && (rel_right_y < ride_ymax && rel_right_y > ride_ymin) 
		&& right_x_pos && rel_right_depth <=2000)
	{
		DBOUT("Ride played \n");
		mciSendString(TEXT("play C:\\Users\\Nirav\\Desktop\\rightCrashDrum-small.WAV"), NULL, 0, NULL);
	}

	if ((rel_right_x < high_hat_xmax && rel_right_x > high_hat_xmin) && (rel_right_y < high_hat_ymax && rel_right_y > high_hat_ymin)
		&& right_y_pos && rel_right_depth <= 2000)
	{
	...
	if ((rel_right_x < crash_left_xmax && rel_right_x > crash_left_xmin) && 
		(rel_right_y < crash_left_ymax && rel_right_y > crash_left_ymin) && right_y_pos)
	{
